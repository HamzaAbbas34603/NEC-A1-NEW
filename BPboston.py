...
X = boston_data_normalized.drop('MEDV', axis=1).values
y = boston_data_normalized['MEDV'].values
    ...
    def fit(self, X, y, epochs, batch_size=32, learning_rate_decay=0.1):
        ...
        for epoch in range(epochs):
            for _ in range(0, len(X_train), batch_size):
                ...
                # Print values during training
                if epoch % 10 == 0:
                    print(f"Epoch {epoch}/{epochs} - Training Error: {training_error}, Validation Error: {validation_error}, Training MAPE: {training_mape}, Validation MAPE: {validation_mape}")
...
layers = [13, 10, 15, 1]  # Input layer: 13 features, Hidden layers: 10, 8, Output layer: 1 unit
learning_rate = 0.01
momentum = 0.9
activation = 'sigmoid'
